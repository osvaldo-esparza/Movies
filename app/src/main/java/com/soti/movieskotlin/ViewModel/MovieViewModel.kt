...
*/
    ...
    fun onCreate(page:Int){
        ...
        viewModelScope.launch {
            try {
                val result = repo.getUpcomingMovies(1)
                result.let {
                    _upcomingMovies.postValue(it)
                }

                val top = repo.getTopRatedMovies(1)
                top.let {
                    _topRatedMovies.postValue(it)
                }

                val popular = repo.getPopularMovies(page)
                popular.let {
    ...
    fun onPopularListEndReached(page: Int,list: String) {
        // Lógica para manejar el evento de llegar al final del RecyclerView
        // Por ejemplo, cargar más películas populares
        loadMorePopularMovies(page,list)
    }

    private fun loadMorePopularMovies(page: Int, list:String) {
        ...
        viewModelScope.launch {
            try {
                ...
                {
                    "POPULAR"->{
                        val newPopularMovies: MovieList = repo.getPopularMovies(page)
                    ...
                    "TOP"->{
                        val newTopMovies: MovieList = repo.getTopRatedMovies(page)
                    ...
                    "UP"->{
                        val newUpMovies: MovieList = repo.getUpcomingMovies(page)
